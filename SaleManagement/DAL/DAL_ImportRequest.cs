...
namespace DAL
{
    public class DAL_ImportRequest:General
    {
        ...
        public bool deleteImportRequest(ImportRequest import)
        {
            ...
            if (ConnectionState.Closed == conn.State)
                conn.Open();
            string sql = "Delete from ImportRequest where Id='" + import.Id + "' ";
            SqlCommand cmd = new SqlCommand(sql, conn);
            try
            {
                if (cmd.ExecuteNonQuery() > 0)
        ...
        public DataTable searchImportRequest(ImportRequest import)
        {
            DataTable dt = new DataTable();
            if (ConnectionState.Closed == conn.State)
                conn.Open();
            SqlCommand cmd = new SqlCommand("select Id, ImportDate, Note, Status, SupplierId, EmployeeId from ImportRequest where Id=N'" + import.Id + "'", conn);
            try
            {
                SqlDataReader rd = cmd.ExecuteReader();
                dt.Load(rd);
