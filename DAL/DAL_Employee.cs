...
namespace DAL
{
    public class DAL_Employee : General
    {
        ...
        public bool updateEmployee(Employee employee)
        {
            ...
            if (ConnectionState.Closed == conn.State)
                conn.Open();
            string sql = "update Employee set Name=N'"+employee.Name+"', Address=N'" + employee.Address + "',Username=N'" + employee.Username + "',Password='" + employee.Password + "',PhoneNumber='" + employee.Phonenumber + "',Email='" + employee.Email + "' where Id='" + employee.Id + "'";
            SqlCommand cmd = new SqlCommand(sql, conn);
            try
            {
                if (cmd.ExecuteNonQuery() > 0)
        ...
        public bool updateEmployeetinhtrang(string username)
        {
            ...
            if (ConnectionState.Closed == conn.State)
                conn.Open();
            string sql = "update Employee set tinhtrang=1 where Username='" + username + "'";
            SqlCommand cmd = new SqlCommand(sql, conn);
            try
            {
                if (cmd.ExecuteNonQuery() > 0)
