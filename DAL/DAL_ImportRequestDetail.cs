...
namespace DAL
{
    public class DAL_ImportRequestDetail:General
    {
        ...
        public bool deleteImportRequestDetail(int id)
        {
            ...
            if (ConnectionState.Closed == conn.State)
                conn.Open();
            string sql = "Delete from ImportRequestDetail where Id='" + id + "' ";
            SqlCommand cmd = new SqlCommand(sql, conn);
            try
            {
                if (cmd.ExecuteNonQuery() > 0)
        ...
        public DataTable searchImportRequestDetail(int id)
        {
            DataTable dt = new DataTable();
            if (ConnectionState.Closed == conn.State)
                conn.Open();
            SqlCommand cmd = new SqlCommand("select Id, ImportRequestId, ProductId, Quantity, Serial from ImportRequestDetail where Id=N'" + id + "'", conn);
            try
            {
                SqlDataReader rd = cmd.ExecuteReader();
                dt.Load(rd);
